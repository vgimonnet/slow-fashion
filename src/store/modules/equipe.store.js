...
export default {
  ...
  actions: {
    async getEquipes ({ commit }) {
      try {
        const equipes = await axios.get('https://slow-fashion.api.vgimonnet.fr/equipes');
    ...
    async getEquipe ({ commit }, id) {
      try {
        const equipe = await axios.get(`https://slow-fashion.api.vgimonnet.fr/equipes/${ id }`);      
    ...
    async createEquipe({ commit }, payload) {
      try {
        const equipe = await axios.post('https://slow-fashion.api.vgimonnet.fr/equipes', payload);
        await commit('EQUIPE', equipe.data);
    ...
    async updateEquipe({ commit }, payload) {
      try {
        const equipe = await axios.put(`https://slow-fashion.api.vgimonnet.fr/equipes/${ payload.id }`, payload.data);
        await commit('EQUIPE', equipe.data);
    ...
    async deleteEquipe({ commit }, id) {
      try {
        await axios.delete(`https://slow-fashion.api.vgimonnet.fr/equipes/${ id }`);
