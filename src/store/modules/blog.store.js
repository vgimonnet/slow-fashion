...
export default {
  ...
  actions: {
    ...
    async getBlog ({ commit }, id) {
      try {
        const blog = await axios.get(`http://localhost:8000/blogs/${ id }`);      
    ...
    async createBlog({ commit }, payload) {
      try {
        const blog = await axios.post('http://localhost:8000/blogs', payload);
        await commit('BLOG', blog.data);
    ...
    async updateBlog({ commit }, payload) {
      try {
        const blog = await axios.put(`http://localhost:8000/blogs/${ payload.id }`, payload.data);
        await commit('BLOG', blog.data);
