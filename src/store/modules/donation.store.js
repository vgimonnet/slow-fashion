...
export default {
  ...
  actions: {
    async getDonation ({ commit }) {
      try {
        const donations = await axios.get('https://slow-fashion.api.vgimonnet.fr/donation');
    ...
    async updateDonation({ commit }, payload) {
      try {
        const donation = await axios.put(`https://slow-fashion.api.vgimonnet.fr/donation`, payload.data);
